        ...
        if (name && name.length > 0) {
          ...
          async.each(words, function (word, cb) {
            ...
            for (var i = 1; i <= word.length; i++) {
              ...
              prefixes.push({ prefix: (ns + prefix), prefixRaw: prefix, type: 'normal' });
              if (!(i >= (word.length - 1))) {
                var postPrefix = word.substring(i);
                prefixes.push({ prefix: (ns + postPrefix), prefixRaw: postPrefix, type: 'inner' });
              }
            }
            async.each(prefixes, function (prefixEntry, cb) {
              if (prefixEntry.prefix.trim() === '') {
                  return cb(null);
              }
              self.conn.sadd({key: self.app + ':' + options.ns + ':keys', value: prefixEntry.prefix}, function (err) {
                ...
                if (!err) {
                  self.conn.sadd({key: self.app + ':' + options.ns + ':' + id, value: { key: prefixEntry.prefix, value: sortName + ':' + id }}, function (err) {
                    err && cb(err);
                    if (!err) {
                      var score = sortName.toLowerCase().indexOf(prefixEntry.prefixRaw);
              ...
              if (!err) {
                ...
                self.conn.sadd({key: self.app + ':' + options.ns + ':' + id, value: { key: ns + word, value: sortName + ':' + id }}, function (err) {
                  err && cb(err);
                  if (!err) {
                    var score = sortName.toLowerCase().indexOf(word);
